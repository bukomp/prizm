...
Files under the harness directories that carry a `# GENERATED by transpile.sh` marker must never be edited by hand — edit the def and re-run the build. The only hand-maintained files in harness directories are context-file templates (`gemini/GEMINI.md`, `opencode/AGENTS.md`, `codex/AGENTS.md`), the one artifact type the transpiler does not generate.

Each top-level harness directory corresponds to one harness, and generated artifacts for that harness live only in its directory (the transpiler creates subdirectories as needed):

- `claude/` — Claude Code (agents, skills, commands)
- `gemini/` — Gemini CLI (agents, skills, commands, GEMINI.md)
- `opencode/` — opencode (agents, skills, commands, AGENTS.md)
- `codex/` — OpenAI Codex CLI (agents, skills, prompts, AGENTS.md)
